    ...
    def handle_begin(self, line):
        ...
        with self.players_lock:
            ...
            if player.get_registered_user() and player.get_welcome_msg():
                self.game.rcon_say("" % (player_name, player_id))
                self.game.rcon_tell(player_num, "" % (player_name, player.roles[player.get_admin_role()], player.get_last_visit(), player.get_num_played()), False)
                ...
                self.game.rcon_tell(player_num, "" % (player_name, player_id))
                player.disable_welcome_msg()
...
class Player(object):
    ...
    def __init__(self, player_num, ip_address, guid, name, auth=''):
        ...
        if info.country:
            self.country = "%s - %s, %s (%s)" % (info.city, info.region_iso_code, info.country_name, info.country)
